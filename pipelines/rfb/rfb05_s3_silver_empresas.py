...
logger = get_logger("rfb_silver_empresas")
...
TAMANHO_LOTE = int(os.getenv("RFB_EMPRESAS_BATCH_SIZE", "500000"))
...
MAPEAMENTO_COLUNAS = {
    "f0": "cnpj_basico",
    "f1": "razao_social",
    "f2": "natureza_juridica",
    "f3": "qualificacao_responsavel",
    "f4": "capital_social",
    "f5": "porte_empresa",
    "f6": "ente_federativo_responsavel"
}

SCHEMA_EMPRESAS = {v: pl.String for v in MAPEAMENTO_COLUNAS.values()}
SCHEMA_EMPRESAS["natureza_juridica"] = pl.Int32
SCHEMA_EMPRESAS["qualificacao_responsavel"] = pl.Int32
SCHEMA_EMPRESAS["porte_empresa"] = pl.Int8
SCHEMA_EMPRESAS["capital_social"] = pl.Float64
SCHEMA_EMPRESAS["referencia_mes"] = pl.Int32
SCHEMA_EMPRESAS["_source_file"] = pl.String
SCHEMA_EMPRESAS["_inserted_at"] = pl.Datetime("us", "UTC")
...
def transformar_lote(df_lote: pl.DataFrame) -> pl.DataFrame:
    return df_lote.rename(MAPEAMENTO_COLUNAS).with_columns([
        ...
        pl.col("razao_social").str.strip_chars().replace("", None),
        pl.col("ente_federativo_responsavel").str.strip_chars().replace("", None),
        pl.col("natureza_juridica").cast(pl.String).str.strip_chars().cast(pl.Int32, strict=False),
        pl.col("qualificacao_responsavel").cast(pl.String).str.strip_chars().cast(pl.Int32, strict=False),
        pl.col("porte_empresa").cast(pl.String).str.strip_chars().cast(pl.Int8, strict=False),
        pl.col("capital_social").str.replace(",", ".").str.strip_chars().cast(pl.Float64, strict=False)
    ])
        ...
        try:
            ...
            try:
                for batch in parquet_file.iter_batches(batch_size=TAMANHO_LOTE):
                    ...
                    df_lote = df_lote.select(list(SCHEMA_EMPRESAS.keys()))
                    validar_schema_basico(df_lote, SCHEMA_EMPRESAS)
...
def processar_empresas_seguro(s3_client):
...
def executar():
    ...
    processar_empresas_seguro(s3_client)
